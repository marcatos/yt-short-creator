...
function connectErrorRedirect(
  request: NextRequest,
  reason: string,
): NextResponse {
  const response = NextResponse.redirect(
    new URL(`/connect?instagram=${encodeURIComponent(reason)}`, request.url),
  );
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  const { connectInstagram, logger } = getContainer();
  ...
  if (oauthError || !code || !stateValid) {
    logger.warn("Instagram OAuth callback rejected", {
      oauthError,
  ...
  try {
    ...
    const response = NextResponse.redirect(
      new URL(`/connect?instagram=connected&username=${encodeURIComponent(account.username)}`, request.url),
    );
    response.cookies.delete(OAUTH_STATE_COOKIE);
    return response;
  } catch (error) {
