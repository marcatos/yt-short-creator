  ...
  return async ({ analysis }) => {
    ...
    const response = await deps.llm.complete({
      system: [
        ...
        "Descriptions: hook (2-3 lines) + story paragraphs separately; race info and hardware are appended by the system — do NOT invent hardware specs.",
        "Thumbnail text must be VERY short; prefer universal (P18 → P8) when it works in both languages.",
        "CTA: brief and natural. Hashtags: few and relevant.",
      ].join("\n"),
